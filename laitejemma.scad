...
$box_hide = "";
$box_remove = "remove fingerslot slot";
$box_keep = "keep";
...
module slidebox(box, lock_rod_y) {
    ...
    rotate(-cutaxis) move(-cutpos) partition([500,500,500],spread=spread, gap=2, cutsize=cutsize, cutpath="jigsaw", $fn=24, $slop=0.1) move(cutpos) rotate(cutaxis) differ() {
        cuboid(box, chamfer=box_chamfer, anchor=BOTTOM) {
            if ($idx == 1) {
                down(box_chamfer+2*tol) children();
            }

            // Lock rod opening
            back(lock_rod_y) align(TOP+LEFT, inside=true) tag("remove") {
                // Rod hole
            ...
            // Lock rod
            up(2*spread) align(TOP+LEFT, inside=true) tag("rod") {
...
module render_box(box, slots, fingerslots, lock_rod_y = 0) {
    slidebox(box, lock_rod_y) {
        ...
        $name = "chassis";
        children();
        tag("slot") align(TOP, inside=true) for (raw = slots) {
            // Making a struct of it
            s = struct_set([], raw);
            $name = struct_val(s, "name", "slot");
        ...
        // Fingerslots
        align(TOP, inside=true) tag("fingerslot") for (slot_pos = fingerslots) {
